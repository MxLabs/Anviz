...
namespace Anviz.SDK
{
    public class AnvizManager : IDisposable
    {
        ...
        public AnvizManager(string host, ulong deviceId, int port = 5010, int timeout = 3)
        {
            ...
            this.host = host;
            this.port = port;
            this.timeout = timeout * 10;//loop waits 100msecs everytime multiply by 10 to get seconds
        }
        ...
        private byte[] SendCommand(byte command, ulong deviceId, byte[] data, ushort dataLength, int responseLength)
        {
            ushort crc = 0x0000;
            ...
            {
                commandBytes[i + 8] = i < data.Length ? data[i] : (byte)0;
            }
            ...
            {
                socket.Send(payload);
                while (socket.Available < responseLength)
                {
                    if (timer > timeout)
                    {
        ...
        {
            byte[] response = SendCommand(GET_RECORD_INFO, deviceId, new byte[] { }, 0, 29);
            ...
            if (response != null)
            {
                ...
                deviceStatistic.NewRecordAmount = (int)ReadBytes(SplitBytes(parsed.DATA, 15, 3));
            }
            return deviceStatistic;
        }

        public List<Record> DownloadRecords(int recordAmount, bool onlyNewRecords)
        {
            List<Record> records = new List<Record>();
            bool isFirst = true;
            while (recordAmount > 0)
            {
                int leftRecord = Math.Min(25, recordAmount);
                byte package = (byte)(isFirst ? 1 : 0);
                byte[] data = new byte[] { onlyNewRecords ? (byte)2 : package, (byte)leftRecord };
                byte[] response = SendCommand(GET_ALL_RECORDS, deviceId, data, 2, 12 + leftRecord * 14);
                if (response == null)
                {
                    return null;
                }
        ...
        public List<UserInfo> GetEmployeesData(int userAmount)
        {
            ...
            while (userAmount > 0)
            {
                int leftRecord = Math.Min(12, userAmount);
                byte package = (byte)(isFirst ? 1 : 0);
                byte[] data = new byte[] { package, (byte)leftRecord };
                byte[] response = SendCommand(GET_STAFF_DATA, deviceId, data, 2, 12 + leftRecord * 30);
                if (response == null)
                {
                    return null;
                }
        ...
        {
            byte[] response = SendCommand(GET_TCP_PARAMETERS, deviceId, new byte[] { }, 0, 38);
            if (response != null)
            {
                Response parsed = GenerateResponse(response);
        ...
        {
            byte[] response = SendCommand(GET_DEVICE_SN, deviceId, new byte[] { }, 0, 15);
            if (response != null)
            {
                Response parsed = GenerateResponse(response);
                if (parsed.RET == ACK_SUCCESS)
        ...
        {
            byte[] response = SendCommand(GET_DEVICE_TYPE, deviceId, new byte[] { }, 0, 19);
            if (response != null)
            {
                Response parsed = GenerateResponse(response);
                if (parsed.RET == ACK_SUCCESS)
        ...
        public bool ClearNewRecords()
        {
            ...
            byte[] response = SendCommand(CLEAR_NEW_RECORDS, deviceId, data, 4, 11);
            if (response != null)
            {
                Response parsed = GenerateResponse(response);
                if (parsed.RET == ACK_SUCCESS)
